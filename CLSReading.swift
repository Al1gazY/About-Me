...
extension CLSReading {
    init?(_ clsData: Data,
        ...
        let isHex = string.allSatisfy { $0.isHexDigit }
        // BPS (18 chars/9 bytes or more if Status present) or ICP (10 chars/5 bytes or more)
        if isHex && (string.count >= 10) && !string.contains("W") && !string.contains("Z") && !string.contains("x") {
             // Basic parsing for print out
             ...
             if !data.isEmpty {
                 ...
                 // Try BPS (min 7 bytes: Flags + 3 SFloats)
                 if data.count >= 7 {
                     // Check if it's likely BPS (flags usually low for our case)
                     // Example: 04 A1 12 50 11 9E 11 ...
                     // Byte 1,2: Sys
                     // Byte 3,4: Dia
                     // Byte 5,6: Mean
                 ...
                 // ICP (Flags + 1 SFloat + ...)
                 if data.count >= 3 && data.count < 7 { // ICP is usually shorter but has similar flags structure
                      // Byte 1,2: ICP
             ...
             // Return dummy to prevent crash/error, but we handled the print
             self = CLSReading(
                DC1: 0, DC2: 0, AC1: 0, AC2: 0, pktID: 0, rtPacket: string, aclReading: nil
             )
             return
        }
